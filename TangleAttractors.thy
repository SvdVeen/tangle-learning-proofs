...
(** A version of the attractor step where we include the strategy. *)
inductive attractor_step_strat :: "'v set \<Rightarrow> 'v strat \<Rightarrow> 'v set \<Rightarrow> 'v strat \<Rightarrow> bool" where
  own: "\<lbrakk>x \<in> V\<^sub>\<alpha>-S; (x,y) \<in> E; y \<in> S\<rbrakk> \<Longrightarrow> attractor_step_strat S \<sigma> (insert x S) (\<sigma>(x\<mapsto>y))"
| opponent: "\<lbrakk>x \<in> V\<^sub>\<beta>-S; \<forall>y. (x,y) \<in> E \<longrightarrow> y \<in> S\<rbrakk> \<Longrightarrow> attractor_step_strat S \<sigma> (insert x S) \<sigma>"
| escape: "\<lbrakk>t \<in> T; t-S \<noteq> {}; opponent_escapes t \<noteq> {}; opponent_escapes t \<subseteq> S;
            player_tangle_strat t \<sigma>'\<rbrakk> \<Longrightarrow> attractor_step_strat S \<sigma> (S \<union> t) ((\<sigma>' |` (t-A)) ++ \<sigma>)"

definition attractor_step_strat_I :: "'v set \<Rightarrow> 'v strat \<Rightarrow> bool" where
  "attractor_step_strat_I S \<sigma> \<equiv>
...
lemma attractor_step_strat_mono: "attractor_step_strat S \<sigma> S' \<sigma>' \<Longrightarrow> S \<subset> S'"
  by (induction rule: attractor_step_strat.induct) auto

lemma attractor_step_strat_I_base:
  "attractor_step_strat_I A Map.empty"
  unfolding attractor_step_strat_I_def
  using origin_in_lasso by fastforce

lemma attractor_step_strat_I_step:
  "attractor_step_strat S \<sigma> S' \<sigma>' \<Longrightarrow> A\<subseteq>S \<Longrightarrow> attractor_step_strat_I S \<sigma> \<Longrightarrow> attractor_step_strat_I S' \<sigma>'"
proof (induction rule: attractor_step_strat.induct)
...
next
  ...
  hence \<sigma>_strat: "strategy_of V\<^sub>\<alpha> \<sigma>" and
    ...
    \<sigma>_forces_A_or_wins: "\<forall>x\<in>S. \<forall>xs ys. lasso_from_node (induced_subgraph V\<^sub>\<alpha> \<sigma>) x xs ys
      \<longrightarrow> set (xs @ ys) \<inter> A \<noteq> {} \<or> winning_player ys"
    unfolding attractor_step_strat_I_def by auto
