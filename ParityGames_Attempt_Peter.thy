...
begin
  ...
  lemma lasso_from_node'I: "lasso_from_node v xs1 xs2 \<Longrightarrow> lasso_from_node' v (xs1@xs2)"
    apply (simp add: lassos_equiv) by blast
    
  lemma lasso_from_node'_nE[simp]: "\<not>lasso_from_node' v []"  
    by (auto simp: lasso_from_node'_def)
  ...
  lemma finite_graph_lasso_always:  "finite E \<Longrightarrow> \<forall>v. E``{v} \<noteq> {} \<Longrightarrow> \<exists>xs. cycle_from_node x xs"
  proof -
    fix x
    assume fin: "finite E" and succ: "\<forall>v. E``{v} \<noteq> {}"
...
lemma subgraph_lasso: "E' \<subseteq> E \<Longrightarrow> cycle_from_node E' v vs \<Longrightarrow> cycle_from_node E v vs"
  ...
  lemma subgraph_lasso_from_node': "E'\<subseteq>E \<Longrightarrow> lasso_from_node' E' v xs \<Longrightarrow> lasso_from_node' E v xs"
    by (metis lasso_from_node'_def subgraph_path)
...
lemma lasso_from_node_imp_path: "lasso_from_node E y xs ys \<Longrightarrow> (\<exists>z. path' E y (xs@ys) z)"
...
lemma aa: "dom \<sigma> \<inter> dom \<sigma>' = {} \<Longrightarrow> x \<in> dom \<sigma> \<Longrightarrow> lasso_from_node (induced_by_strategy (\<sigma> ++ \<sigma>')) x xs ys
   \<Longrightarrow> lasso_from_node (induced_by_strategy \<sigma>) x xs ys"
  apply (induction xs arbitrary: x) apply auto sorry

lemma attr_even_subset: "attr_even X Y \<Longrightarrow> X \<subseteq> Y"  
  apply (induction rule: attr_even.induct)
  by auto
  
find_theorems cycle_from_node lasso_from_node'  
  

lemma induced_by_strategy_addD:
  shows "induced_by_strategy (\<sigma> ++ \<sigma>') \<subseteq> induced_by_strategy \<sigma> \<union> E_of_strat \<sigma>'"
...
proof (induction rule: attr_even.induct)
  ...
  then show ?case
    ...
    subgoal for y xs
      ...
      apply (cases xs; auto simp: Cons_eq_append_conv)
      done
    done
next
  case (step Y Y')
  from step.IH obtain \<sigma> where
   EVEN_\<sigma>: "strategy_of V\<^sub>0 \<sigma>"
   and DOM_\<sigma>: "dom \<sigma> \<subseteq> Y"
   and YCLOSED: "induced_by_strategy \<sigma> `` (Y-X) \<subseteq> Y"
...

  {
    ...
    proof (cases)
      assume "y\<in>X" thus ?thesis sorry
    next
      assume "y\<notin>X"
      
      from subgraph_lasso_from_node'[OF induced_by_strategy_addD, OF y_lasso] have 
        y_lasso': "lasso_from_node' (induced_by_strategy \<sigma> \<union> E_of_strat \<sigma>') y xs" .
      then obtain y' xs' where 
        FST_EDGE: "(y,y')\<in>induced_by_strategy \<sigma> \<union> E_of_strat \<sigma>'"  and
        RM_LASSO: "lasso_from_node' (induced_by_strategy \<sigma> \<union> E_of_strat \<sigma>') y' xs'"
        and RML_SS: "set xs' \<subseteq> set xs"
        apply (cases xs; simp)
        apply (drule lasso_from_node'_consD)
        by auto
  
      have "y'\<in>Y" using FST_EDGE
        apply safe
        subgoal 
          using YCLOSED \<open>y\<notin>X\<close> \<open>y\<in>Y'\<close> unfolding Y'_def
          apply -
          apply safe
          apply auto []
          subgoal
            (* TODO: THis is making case distinction (done below in Isar). Move this reasoning there.*)
          
           sorry
          using ind_subgraph by force
